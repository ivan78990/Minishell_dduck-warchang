...
t_comm	*ft_parse_token_for_comm(t_count *cnt, t_list *lst_tkn)
{
	t_list	*lst_redir;
	t_list	*lst_arg;
	t_comm	*new_command;
	...
	{
		cnt->flag = ft_flag_token((char *)lst_tkn->content);
		if (cnt->flag == 1)
		{
...
t_comm	*ft_get_command(t_count *cnt, t_list *lst_token)
{
	t_comm	*new_command;

	new_command = NULL;
	lst_token = ft_get_pos_list_token(cnt->count_drop_token, lst_token);
	new_command = ft_parse_token_for_comm(cnt, lst_token);
	cnt->k = 0;
...
t_list	*ft_get_list_comm(t_data *data, t_list *lst_token)
{
	...
	while (i < (data->count_pipe + 1))
	{
		cnt.z = 0;
		cnt.flag = 1;
		command = ft_get_command(&cnt, lst_token);
